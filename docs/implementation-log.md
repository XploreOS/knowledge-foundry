...
- **Release automation**: release-please manifest mode (`release-please-config.json`, baseline 0.2.0, `node-workspace` + `linked-versions` so all packages version in lockstep) + `.github/workflows/release-please.yml` — merging the release PR publishes every not-yet-published workspace via `scripts/publish-workspaces.mjs`. npm auth switched to OIDC **trusted publishing** (no token secret; provenance automatic; npm ≥ 11.5.1 installed in CI; first publish of any new package is manual because npm only lets existing packages configure a trusted publisher). `RELEASE_PLEASE_TOKEN` fine-grained PAT lets the action open release PRs and makes CI run on them.
